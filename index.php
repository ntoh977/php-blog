...
if (@$userdata['user_login']){ ?>
  <div style="position: fixed;
    z-index: 99999999999999;left: 80%;">
      <div id="top"> <a href="stranic.php">в админку</a></div>
<div id="top"> <a href="add.php">редактировать записи</a></div>
<div id="top"> <a href="delite.php">удалить записи и изображение</a></div>
  <div id="top"> <a href="add_posts.php">добавить запись и изображение</a></div>
</div>
<?php } ?>
    ...
    <section class="site-section-hero bg-image" style="background-image: url('<?php echo "images/".$row['imag']." " ?> ');"  data-stellar-background-ratio="0.5" id="section-home">
        <div class="row justify-content-center align-items-center">
          <div class="col-md-7 text-center">
            <h1 class="text-white heading text-uppercase" data-aos="fade-up"><?php echo $row['title'] ?></h1>
            <p class="lead text-white mb-5" data-aos="fade-up" data-aos-delay="100"><?php echo $row['texts_h'] ?></p>
...
<?php } ?>
 ...
 $result = mysqli_query($db, "SELECT * FROM `slider`");
    while($row = mysqli_fetch_array($result)) { ?>
        ...
        <?php echo "<a href='images/".$row['slider_image']."' " ?> data-fancybox="gallery"class="d-block photo-item">
       
          <?php echo "<img src='images/".$row['slider_image']."' " ?> alt="Image" class="img-fluid">
 ...
 <?php
  ...
  $start=0;
   $limit=7;
      ...
      $t=mysqli_query($conn,"select * from `table`");
...
<?php
    ...
    while($row = mysqli_fetch_array($query)) {
             $id = $row['id']; ?>
                <div id = 'r' class="col-md-12 mb-4" data-aos="fade-up">
...
<div class="mr-0 mr-md-5 mb-3 img-wrap">
  ...
  $t = "images/".$row['imag']."";
            if(file_exists($t)) { ?>
        <?php echo "<img src='images/".$row['imag']."' " ?>alt="Image"  class="img-fluid">
           <?php }else {
            echo "no image";
        };
           ...
           ?>
                    ...
                    <div>
                      <h2 class="mt-0 mb-2"><?php  echo "<a href='single.php?id=$id'>".$row['title'] ."</a>"; ?></h2>
                      <div class="meta mb-3">Posted by Ben Jones on <a href="#"><?php echo  $row['cifre']; ?></a></div>
                     <?php  $text_obrez = "<p>".$row['texts']."</p>";
      echo mb_strimwidth($text_obrez, 0, 150, "..."); ?>
                    </div>
                  </div>
                </div>
...
// window.history.pushState("object or string", "Title", "/ <?php
//  for($i=1;$i <= $page;$i++){
//  ?>
//    ?id=<?php echo $i ?> "><?php echo $i;?>
//   <?php
//  }
//   ?>")";
</script>
